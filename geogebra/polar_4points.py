...
def locate(
    ...
    c3 = insec(p1, d1, p4, d4)
    print(f"圆1与圆2的交点为：{c1}")
    print(f"圆1与圆3的交点为：{c2}")
    print(f"圆1与圆4的交点为：{c3}")
    for r, t in c1:
        for r2, t2 in c2:
            for r3, t3 in c3:
                if (
                    r == r2
                    and r == r3
                    and round(t, 0) == round(t2, 0)
                    and round(t, 0) == round(t3, 0)
                ):
                    print(f"四圆确定的一点为：{(r, t)}")
                    return (r, t)
    print("无交点")
    return None
...
def solve_nonlineq(p0, p1, p2, p3, alpha01, alpha02, alpha12, alpha03):
    ...
    # 余弦定理方程组
    h0, h1, h2, h3 = sy.symbols("h0 h1 h2 h3")
    eq = [
        cos_therom(h0, h1, d01, alpha01),
        cos_therom(h0, h2, d02, alpha02),
        cos_therom(h1, h2, d12, alpha12),
        cos_therom(h0, h3, d03, alpha03),
    ]
    # 得到非线性方程组的解析解
    rst = sy.nonlinsolve(eq, [h0, h1, h2, h3])
    # 过滤结果，筛选出合理的点位
    points = []
    for r in rst:
        x, y, z, w = r
